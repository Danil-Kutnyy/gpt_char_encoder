...
def get_batch_qa(split, batch_size=batch_size):
	pad_val = 2
	xs = []
	ys = []
	masks = []
...
class GPTLanguageModel(nn.Module):
	...
	def generate(self, idx, answer=None):
...
for i in range(64):
	...
	cut_b = 128
	cut_c = cut_a*cut_b
	text = tokenizer.decode(xb[cut_a].tolist(),chunked=True)[0:cut_b]
	text_seq = tokenizer.decode(xb[cut_a].tolist())[0:cut_b]
	text_pred = text[1:] + [' ']
	#print(xb[0])
	...
	#print(torch.argmax(logits_original[0]))

	masked_loss = loss_raw * m.view(-1)  # Element-wise multiplication
	#print(masked_loss[:45])
	ind_cut = text_seq.find('    ')
	text_seq = text_seq[:ind_cut]
	#print(text_seq)
	#print('tokens:')
	loss1 += masked_loss.sum().item()
	#[print(a,'-',b,'-',f'{c.item():.2f}','-',tokenizer.decode([torch.argmax(d).item()])) for a,b,c,d in zip(text, text_pred, masked_loss[cut_c:cut_c+cut_b], logits_original[cut_c:cut_c+cut_b]) if c.item() != 0.0]
	for a,b,c,d in zip(text, text_pred, masked_loss[cut_c:cut_c+cut_b], logits_original[cut_c:cut_c+cut_b]):
		if c.item() != 0.0:
			acc1.append([b,tokenizer.decode([torch.argmax(d).item()])])
	...
	loss2 += masked_loss.sum().item()
	#[print(a,'-',b,'-',f'{c.item():.2f}','-',tokenizer.decode([torch.argmax(d).item()])) for a,b,c,d in zip(text, text_pred, masked_loss[cut_c:cut_c+cut_b], logits_char_encoder_add[cut_c:cut_c+cut_b]) if c.item() != 0.0]
	for a,b,c,d in zip(text, text_pred, masked_loss[cut_c:cut_c+cut_b], logits_char_encoder_add[cut_c:cut_c+cut_b]):
		if c.item() != 0.0:
			acc2.append([b,tokenizer.decode([torch.argmax(d).item()])])
	#print('-----')
...
for a,b in acc1:
	if a == b:
		acc1_true+=1
acc1 = acc1_true/len(acc1)

acc2_true = 0
for a,b in acc2:
	if a == b:
		acc2_true+=1
acc2 = acc2_true/len(acc2)

print('acc original:',acc1)
print('acc char:',acc2)
print('original:',loss1)
print('char:',loss2)
